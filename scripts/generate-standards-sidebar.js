...
const STANDARD_LABELS = {
  ...
  'hcs-25': 'HCS-25: AI Trust Score Methodology',
};
 ...
 */
function buildStandardSidebar(hcsId, standardPath, isDirectory, hasSiblingFile) {
  ...
  if (!hasIndex) {
    // Directory without index - use autogenerated. When a sibling
    // hcs-X.md exists, link the category to that main spec doc so the
    // standard stays reachable from the sidebar.
    const category = {
      type: 'category',
      label,
      collapsible: true,
    ...
    if (hasSiblingFile) {
      category.link = { type: 'doc', id: `standards/${hcsId}` };
    }
    return category;
  }
...
function generateStandardsSidebar() {
  ...
  for (const entry of entries) {
    ...
    if (standardsMap.has(hcsId)) {
      ...
      if (isDirectory && !existing.isDirectory) {
        ...
        // Directory seen first, sibling file second: remember the hybrid
        standardsMap.set(hcsId, { ...existing, hasSiblingFile: true });
      }
  ...
  const sidebarItems = standards.map((std) =>
    buildStandardSidebar(std.id, std.path, std.isDirectory, std.hasSiblingFile)
  );
