...
const HCS25_FLAT_DOC_SIDEBAR = [
  {
    suffix: '/docs/standards/hcs-25/adapters.md',
    position: 3,
    label: 'Adapter overview',
  },
  {
    suffix: '/docs/standards/hcs-25/signals.md',
    position: 4,
    label: 'Signal overview',
  },
  {
    suffix: '/docs/standards/hcs-25/simple-evals.md',
    position: 5,
    label: 'Simple evals overview',
  },
];

/**
 * Inject short sidebar_label frontmatter into HCS-25 appendix docs so the
 * autogenerated sidebar stays scannable. Upstream titles follow the pattern
 * "HCS-25 (Adapter|Signal|Appendix): Short name (Informative)"; the verbose
 * prefix and suffix would otherwise appear on every sidebar entry.
 */
function fixHcs25SidebarLabels(content, destPath) {
  const normalized = String(destPath).replace(/\\/g, '/');
  if (!/\/docs\/standards\/hcs-25\/.+\.md$/.test(normalized)) {
    return content;
  }
  if (/\/docs\/standards\/hcs-25\/(?:adapters|signals)\/index\.md$/.test(normalized)) {
    return content;
  }

  const flatOverride = HCS25_FLAT_DOC_SIDEBAR.find((entry) =>
    normalized.endsWith(entry.suffix),
  );

  let label = flatOverride?.label;
  if (!label) {
    const titleMatch = content.match(
      /^#\s+HCS-25 \([^)]+\):\s*(.+?)\s*(?:\(Informative\))?\s*$/m,
    );
    label = titleMatch?.[1];
  }
  if (!label) {
  ...
  let updated = content;
  if (/^sidebar_label:/m.test(updated)) {
    updated = updated.replace(
      /^sidebar_label:.*$/m,
  ...
  if (flatOverride && !/^sidebar_position:/m.test(updated)) {
    updated = updated.replace(
      /^---\n/,
      `---\nsidebar_position: ${flatOverride.position}\n`,
    );
  }
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      if (entry.name.endsWith('.md') || entry.name.endsWith('.mdx')) {
        ...
        escapedContent = fixHcs25SidebarLabels(escapedContent, destPath);
...
const CATEGORY_MAP = {
  ...
  'hcs-25': { category: 'AI & Communication', features: ['Trust signal schemas', 'Deterministic adapters', 'Composite AI trust score'] },
};
