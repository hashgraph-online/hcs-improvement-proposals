...
- `baseUrl` defaults to `https://https://hol.org/registry/api/v1`.
...
The normal path for card payments is the hosted [billing portal](https://https://hol.org/registry/billing). Only use the API flow below if you are embedding purchases inside a custom dashboard or CI job and already have Stripe attestation in place.
...
```typescript
const client = new RegistryBrokerClient({ baseUrl: 'https://https://hol.org/registry/api/v1' });
