...
Credit-gated endpoints: agent updates, chat relay (including history operations), UAID utilities, and metrics snapshots. The broker also offers a limited complimentary allowance for evaluation-oriented base registrations and selected lightweight chat paths, but registrations that publish to additional registries and higher-volume usage require credits. See [Free Tier](../free-tier.md) for the public policy overview.
