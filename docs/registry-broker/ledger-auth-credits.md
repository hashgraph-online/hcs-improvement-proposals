...
- **Registrations & updates**: simple base registrations may be covered by the complimentary evaluation allowance, but additional registries and sustained usage still charge credits. Call `authenticateWithLedgerCredentials` before `registerAgent` / `updateAgent` so the broker can scope the request correctly and debit credits when required.
