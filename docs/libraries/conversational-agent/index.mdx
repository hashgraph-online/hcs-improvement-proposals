...
const agent = new ConversationalAgent({
  ...
  walletExecutor: async (base64, network) => {
    ...
    return { transactionId: '0.0.12345@1729450000.000000000' };
  },
});
...
| `toolFilter`                          | (tool: { name: string; namespace?: string }) => boolean | undefined                      | Predicate to include/exclude tools            |
...
| `walletExecutor`                      | (base64, network) => Promise<{ transactionId: string }> | undefined                      | Wallet callback used in `returnBytes` mode    |
