...
- `anthropic`: uses Claude models (e.g., `claude-3-7-sonnet-latest`, `claude-3-haiku-20240307`).
- `openrouter`: uses OpenRouter routing (e.g., `openrouter/auto` or a concrete model like `openai/gpt-4o-mini`).
...
const agent = new ConversationalAgent({
  ...
  // 'openrouter/auto' lets OpenRouter route; or choose a concrete model you can access
...
[Learn more about MCP Integration →](./mcp-servers)
...
| `openAIModelName`                     | string               | 'gpt-4o'                     | Model to use (OpenAI/Anthropic/OpenRouter) |
| `llmProvider`                         | 'openai'             | 'anthropic'                  | 'openrouter'                               | 'openai'             | Select LLM provider                       |
...
| `toolFilter`                          | (tool) => boolean    | undefined                    | Predicate to include/exclude tools         |
...
| `disableLogging`                      | boolean              | false                        | Silence logs (still errors)                |
| `scheduleUserTransactionsInBytesMode` | boolean              | undefined                    | Schedule when in bytes mode                |
...
| `entityMemoryProvider`                | 'openai'             | 'anthropic'                  | 'openrouter'                               | llmProvider/'openai' | Provider for entity extraction/resolution |
| `entityMemoryModelName`               | string               | per-provider                 | Model used for entity extraction           |
| `openRouterApiKey`                    | string               | undefined                    | API key for OpenRouter                     |
| `openRouterBaseURL`                   | string               | https://openrouter.ai/api/v1 | Override OpenRouter API base URL           |
