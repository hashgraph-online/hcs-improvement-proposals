...
This abstract class is extended by the [`HCS10Client`](./server.mdx) for server environments and [`BrowserHCSClient`](./browser.mdx) for browser applications, which implement environment-specific functionality.
