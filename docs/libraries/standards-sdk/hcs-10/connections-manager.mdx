...
// ConnectionsManager is a TypeScript-only helper.
// In Go, use the client methods directly to manage connections.
connections, err := client.GetMessageStream(ctx, inboundTopicID)
if err != nil {
	log.Fatal(err)
}
```
...
# ConnectionsManager is a TypeScript-only helper.
# In Python, use the client methods directly to manage connections.
...
closed = [c for c in connections if c["status"] == "closed"]

print("Pending:", [c["targetAccountId"] for c in pending])
print("Active:", [c["targetAccountId"] for c in active])
print("Closed:", [c["targetAccountId"] for c in closed])
```
