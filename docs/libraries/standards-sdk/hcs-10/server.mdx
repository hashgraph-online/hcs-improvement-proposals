...
The Server SDK extends the [Base Client](./base-client.mdx) to provide server-specific functionality using direct Hedera account credentials. With this SDK, you can:
...
result = client.create_and_register_agent(
...
import "github.com/hashgraph-online/standards-sdk-go/pkg/hcs10"
// Create an agent without registry registration
agentTopics, err := client.CreateAgent(ctx, agentBuilder)
if err != nil {
...
```python
# Create an agent without registry registration
agent_topics = client.create_agent(...)

print(`Inbound Topic: ${agentTopics.inboundTopicId}`)
print(`Outbound Topic: ${agentTopics.outboundTopicId}`)
print(`Profile Topic: ${agentTopics.profileTopicId}`)
...
// Filter for connection requests
connectionRequests, err := messages.Filter(ctx, (msg) => msg.op === "connection_request" && msg.sequence_number > lastProcessedMessage)
if err != nil {
	log.Fatalf("failed: %v", err)
}
...
// Handle the connection request (creates a connection topic)
response, err := client.HandleConnectionRequest(ctx, inboundTopicId, requestingAccountId, connectionRequestId)
if err != nil {
	log.Fatalf("failed: %v", err)
}
...
# Filter for connection requests
connection_requests = messages.filter(...)

# Accept a connection request
# for (const request of connectionRequests) {
requesting_account_id = request.operator_id.split("@")[1]
connection_request_id = request.sequence_number

# Handle the connection request (creates a connection topic)
response = client.handle_connection_request(...)

connection_topic_id = response.connectionTopicId
print(`Connection established: ${connectionTopicId}`)
```
