...
# Create an indexed registry (default)
indexedResult = client.create_registry(registry_type=Hcs2RegistryType.INDEXED, ttl=86400)

# Create a non-indexed registry
nonIndexedResult = client.create_registry(registry_type=Hcs2RegistryType.NON_INDEXED, ttl=86400)
```
...
```python
result = client.create_registry(registry_type=Hcs2RegistryType.INDEXED, ttl=3600, use_operator_as_admin=True, use_operator_as_submit=True)

fmt.printf("Registry created with topic ID: %s\n", result.topic_id)

# Create a non-indexed registry
nonIndexedResult = client.create_registry(registry_type=Hcs2RegistryType.NON_INDEXED, ttl=3600)
```
...
```python
registry_topic_id = "0.0.12345"

registerResult = client.register_entry(registryTopicID, target_topic_id="0.0.67890", metadata="https://example.com/metadata.json", memo="Initial registration")

fmt.printf("Entry registered. Sequence number: %d\n", register_result.SEQUENCE_NUMBER)
```
...
```python
# This only works with indexed registries
updateResult = client.update_entry(registryTopicID, uid="1", target_topic_id="0.0.98765", metadata="https://example.com/new-metadata.json", memo="Updated registration")

fmt.printf("Entry updated. New sequence number: %d\n", update_result.SEQUENCE_NUMBER)
```
...
```python
# This only works with indexed registries
deleteResult = client.delete_entry(registryTopicID, uid="1", memo="Entry deleted")

fmt.printf("Entry deleted. Sequence number: %d\n", delete_result.SEQUENCE_NUMBER)
```
...
```python
old_registry_topic_id = "0.0.12345"
new_registry_topic_id = "0.0.67890"

migrateResult = client.migrate_registry(oldRegistryTopicID, target_topic_id=newRegistryTopicID, memo="Migrating to new registry topic")
...
```python
registry = client.get_registry(registryTopicID)

fmt.printf("Registry type: %d, entries: %d\n", registry.REGISTRY_TYPE, len(registry.Entries))
