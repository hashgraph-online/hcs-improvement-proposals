...
client = Hcs2Client(network="testnet", operator_id=os.environ["HEDERA_ACCOUNT_ID"], operator_private_key=os.environ["HEDERA_PRIVATE_KEY"])

created = client.create_registry(ttl=86400, use_operator_as_submit=True)

client.register_entry(created.topic_id, target_topic_id="0.0.2001", metadata="app=chat", "hcs-2")

reg = client.get_registry(created.topic_id)
```
