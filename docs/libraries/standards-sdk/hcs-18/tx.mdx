...
tx = hcs18.build_create_discovery_topic_tx(ttl_seconds=300, admin_key=operatorPublicKey, submit_key=operatorPublicKey)
...
```python
tx = hcs18.build_submit_discovery_message_tx("0.0.555", msg, "optional")
...
```python
msg = hcs18.build_announce_message(account="0.0.123", petal={"name": "A", "priority": 700}, capabilities={"protocols": [][], }, valid_for=300)
```
...
```python
msg = hcs18.build_propose_message(proposer="0.0.1", members=[]{"account": "0.0.2", "priority": 500}}, config={"name": "Demo Flora", "threshold": 2})
```
...
```python
msg = hcs18.build_respond_message(responder="0.0.2", proposal_seq=10, decision="accept")
```
...
```python
msg = hcs18.build_complete_message(proposer="0.0.1", proposal_seq=10, flora_account="0.0.500", topics={"communication": "0.0.600", "transaction": "0.0.601", "state": "0.0.602"})
```
...
```python
msg = hcs18.build_withdraw_message(account="0.0.2", announce_seq=42, reason="going offline")
```
