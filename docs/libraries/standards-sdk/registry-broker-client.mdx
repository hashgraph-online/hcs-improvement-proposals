...
# Standards SDK Registry Broker Client
...
const client = new RegistryBrokerClient({
  ...
  apiKey: process.env.REGISTRY_BROKER_API_KEY,
    ...
    },
    {
      ...
      language: 'go',
      icon: <FaCode />,
      code: `client, err := registrybroker.NewRegistryBrokerClient(registrybroker.RegistryBrokerClientOptions{
...
#### TypeScript custom fetch override
...
console.log(reply.message);`
    },
    {
      ...
      language: 'go',
      icon: <FaBolt />,
      code: `session, err := client.CreateSession(context.Background(), registrybroker.CreateSessionRequestPayload{
	UAID: "uaid:aid:2bnewJwP95isoCUkT5mee5gm212WS76tphHwBQvbWoquRa9kt89UanrBqHXpaSh4AN;uid=anthropic/claude-3.5-sonnet;registry=openrouter;proto=openrouter;nativeId=anthropic/claude-3.5-sonnet",
}, true)
if err != nil {
	panic(err)
...
fmt.Println(reply)`
    },
    {
      ...
      language: 'python',
      icon: <FaBolt />,
      code: `session = client.chat.create_session(
    {
        "uaid": "uaid:aid:2bnewJwP95isoCUkT5mee5gm212WS76tphHwBQvbWoquRa9kt89UanrBqHXpaSh4AN;uid=anthropic/claude-3.5-sonnet;registry=openrouter;proto=openrouter;nativeId=anthropic/claude-3.5-sonnet"
    }
)
...
Seed chat sessions with a UAID (`{ uaid: 'uaid:...' }`) for all integrations. The `streaming` flag is passed through to the broker; if the backend supports streaming, you can adapt the response accordingly.
...
- `standards-sdk/demo/registry-broker/register-agent-moltbook.ts` (register + Moltbook claim details)
...
## Skill Verification (DNS Domain Proof)

The `RegistryBrokerClient` skill APIs support version-scoped verification requests and DNS TXT domain proof.

```typescript
// 1) request paid verification for an exact version
...
// 2) check verification status/signals for that version
const status = await client.getSkillVerificationStatus({
  name: 'demo-skill',
  version: '1.0.0',
});

// 3) create DNS TXT challenge
...
console.log(status.verified, domainProof.signal.ok);
```

## Skill Lifecycle and Preview Status

Use these methods when you need the machine-readable lifecycle contract behind validate-first GitHub Actions, preview pages, and canonical `name@version` status lookups.
...
console.log(status.trustTier, statusByRepo.found, preview.found, previewByRepo.found, previewById.found);
```
...
Use the repo-based methods for pull request validation because they resolve state against `repo`, `skillDir`, and an optional Git `ref` before falling back to published `name@version` records.

### Secretless GitHub OIDC preview upload

Validate-only workflows can upload preview reports without `RB_API_KEY` when the workflow grants `id-token: write`.
...
## UAID DNS Verification (HCS-14)

Use these methods to validate that a UAID is correctly bound to a DNS TXT record (`_uaid.<nativeId>`).
...
// 1) live DNS TXT verification (optionally persist signal to agent metadata)
const verifyResult = await client.verifyUaidDnsTxt({
  uaid,
  persist: true,
});

// 2) read stored status or force live recheck via query flags
const dnsStatus = await client.getVerificationDnsStatus(uaid, {
  refresh: true,
  persist: true,
});

console.log(verifyResult.verified, dnsStatus.profileId, dnsStatus.dnsName);
```
