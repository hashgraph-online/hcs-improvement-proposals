...
```python
# NewClient(config ClientConfig) (*Client, error)
Client.create_state_topic(...)
Client.submit_message(...)
Client.compute_and_publish(...)
Client.validate_topic(topicID string) (bool, *TopicMemo, error)
# Client.GetRecentMessages / GetLatestMessage
Client.calculate_account_state_hash(AccountStateInput) (StateHashResult, error)
Client.calculate_composite_state_hash(CompositeStateInput) (CompositeStateHashResult, error)
# BuildCreateStateTopicTx(CreateTopicOptions) *hedera.TopicCreateTransaction
# BuildStateHashMessageTx(topicID string, StateHashMessage, txMemo string) (*hedera.TopicMessageSubmitTransaction, error)
```
