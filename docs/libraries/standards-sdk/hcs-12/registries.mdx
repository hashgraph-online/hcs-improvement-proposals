...
// Initialize all registries
// client.InitializeRegistries(ctx, ...)
// action: '0.0.456789',
// assembly: '0.0.567890',
// hashlinks: '0.0.678901',
// });

// client.SyncRegistries(ctx, ...)
// console.Log(ctx, ...)
```
...
# await client.syncRegistries();
print('Registry topics:', client.getRegistryTopicIds())
```
...
// Apply cache configuration
// client.ConfigureRegistryCache(ctx, ...)

// Cache statistics
cacheStats, err := client.GetRegistryCacheStats(ctx, )
if err != nil {
...
// client.ConfigureRegistryAudit(ctx, ...)

// Query audit logs
auditLogs, err := client.GetRegistryAuditLogs(ctx, { registry: "hashlinks", operation: ["register", "update"], dateRange: { from: new Date("2024-01-01"), to: new Date("2024-12-31") }, author: "0.0.123456", limit: 100 })
if err != nil {
	log.Fatalf("failed: %v", err)
}
...
// Generate compliance report
complianceReport, err := client.GenerateComplianceReport(ctx, { startDate: new Date("2024-01-01"), endDate: new Date("2024-12-31"), includeMetrics: true, format: "pdf" })
if err != nil {
	log.Fatalf("failed: %v", err)
}
```
...
```python
# Monitor sync health
sync_monitor = new RegistrySyncMonitor(client)
# syncMonitor.on('sync_lag', async (registry, lagMinutes) => {
if lagMinutes > 60:
# console.warn(`Registry ${registry} lag detected: ${lagMinutes} minutes`);
