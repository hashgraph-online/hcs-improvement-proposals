...
import "github.com/hashgraph-online/standards-sdk-go/pkg/hcs12"
// Built-in sanitization is applied during render/template evaluation
rendered, err := templateEngine.Render(ctx, unsafeTemplate, { attributes: { label: "<script>alert(1)</script>" }, })
if err != nil {
...
import "github.com/hashgraph-online/standards-sdk-go/pkg/hcs12"
// Register custom helper
// templateEngine.RegisterHelper(ctx, ...)
// Intl.NumberFormat(ctx, ...)
// style: 'currency',
// currency: currency || 'USD'
// }).format(amount);
// });
...
import "github.com/hashgraph-online/standards-sdk-go/pkg/hcs12"
// Template caching
// templateEngine.ClearCache(ctx, ...)
cacheSize, err := templateEngine.GetCacheSize(ctx, ); // Monitor cache size)
if err != nil {
	log.Fatalf("failed: %v", err)
}

// Precompilation
// templateEngine.Precompile(ctx, ...)
html, err := templateEngine.RenderCompiled(ctx, "counter-template", context)
if err != nil {
	log.Fatalf("failed: %v", err)
}

// Batch rendering
results, err := Promise.All(ctx, [ blockRenderer.render(block1, options1), blockRenderer.render(block2, options2), blockRenderer.render(block3, options3) ])
if err != nil {
	log.Fatalf("failed: %v", err)
}
```
