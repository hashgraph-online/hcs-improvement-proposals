...
// Register block (template is automatically inscribed via HCS-1)
registeredBuilder, err := client.RegisterBlock(ctx, blockBuilder)
if err != nil {
	log.Fatalf("failed: %v", err)
}
blockTopicId, err := registeredBuilder.GetTopicId(ctx, )
if err != nil {
	log.Fatalf("failed: %v", err)
}
...
registered2, err := client.RegisterBlock(ctx, blockBuilder2)
if err != nil {
	log.Fatalf("failed: %v", err)
}
```
...
# .setTitle('My Block')
# .setCategory('widgets')
# .setTemplate(Buffer.from(templateHtml)); 
```
