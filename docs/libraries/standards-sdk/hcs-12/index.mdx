...
try {
  validateBlockRegistration(blockDef);
} catch (error) {
  console.error(error);
}

const checked = safeValidate(actionRegistrationSchema, actionReg);
if (!checked.success) {
...
const wasmInscription = await client.inscribeFile(wasmBuffer, 'counter.wasm');
...
actionBuilder
  .setTopicId(wasmInscription.topic_id)
  .setWasmHash(wasmHash)
  .setHash(infoHash);

// Register the action
const actionRegistration = await client.registerAction(actionBuilder);
const actionTopicId = actionRegistration.getTopicId();
console.log('Action registered:', actionTopicId);
```
...
const blockBuilder = new BlockBuilder()
  ...
  .addAction('increment', actionTopicId)
  .addAction('decrement', actionTopicId)
  .addAction('reset', actionTopicId);

// Register the block
const blockRegistration = await client.registerBlock(blockBuilder);
const blockTopicId = blockRegistration.getTopicId();
...
const [firstBlock] = assembly.blocks;
if (!firstBlock?.definition) throw new Error('No resolved block definition');

const block = {
  id: firstBlock.block_t_id,
  p: 'hcs-12',
  op: 'register',
  name: firstBlock.definition.name,
  version: firstBlock.definition.version,
  template: firstBlock.template || '',
  attributes: firstBlock.definition.attributes,
  t_id: firstBlock.block_t_id,
};
...
const result = await renderer.render(block, {
  ...
  assembly,
  network: 'testnet',
  initialState: { attributes: {}, actionResults: {} },
});
...
```typescript
import { RegistryType } from '@hashgraphonline/standards-sdk';
...
stateManager.onStateChange('counter-1', state => {
  console.log('Counter updated:', state.attributes.count);
});

// Update state after action execution
stateManager.updateBlockState('counter-1', { attributes: { count: 42 } });
```
...
try {
  const result = await wasmExecutor.execute(actionRegistration.build(), {
    method: 'POST',
    params: { operation: 'transfer', amount: 100, to: '0.0.789' },
  });
...
- **[Actions](./actions.mdx)** - Deep dive into WebAssembly modules
- **[Blocks](./blocks.mdx)** - Master UI component creation
- **[Assembly](./assembly.mdx)** - Learn composition patterns
- **[Registries](./registries.mdx)** - Understand discovery systems
- **[Rendering](./rendering.mdx)** - Explore the rendering pipeline
- **[Security](./security.mdx)** - Implement secure HashLinks
- **[WASM](./wasm.mdx)** - WebAssembly execution details
...
- **[GitHub Repository](https://github.com/hashgraph-online/standards-sdk)** - Source code
- **[Examples](https://github.com/hashgraph-online/standards-sdk/tree/main/demo/hcs-12)** - Sample applications
