...
// Execute an action registration
result, err := wasmExecutor.Execute(ctx, actionRegistration, { method: "POST", params: { operation: "transfer", amount: 100, to: "0.0.987654", }, })
if err != nil {
...
// Ensure wasm-bindgen versions match
// In ActionBuilder:
// actionBuilder.SetInterfaceVersion(ctx, ...)

// Verify imports in WASM module
wasmModule, err := WebAssembly.InstantiateStreaming(ctx, wasmResponse)
if err != nil {
	log.Fatalf("failed: %v", err)
}
// console.Log(ctx, ...)
...
# Ensure wasm-bindgen versions match
# In ActionBuilder:
# actionBuilder.setInterfaceVersion('0.2.95');
```
